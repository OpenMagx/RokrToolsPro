...
/*
bool TaskListBox::codeCmp()
{
    //if(cfg->reg_key == QString("198704062437wgjWGJ") )
	//	return true;
	...
	if(cfg->reg_type == "IMSI")
	{
	    step = 3;
		code = gui->usrIMSI;
	}else if(cfg->reg_type == "IMEI")	{
	    step = 4;
		code = gui->usrIMEI;
	}else {
	    step = 4;
	...
	sprintf(codeStr, "%s", (const char*)code.local8Bit() );
    ...
    for(int i=0;i<15;i++)
    {
		...
		n = n*step;
		keyStr[i] = keyList[n];
		key_chk.append(keyStr[i]);
    }		
    if(key_chk == cfg->reg_key)
		return true;
	else
		return false;
}
