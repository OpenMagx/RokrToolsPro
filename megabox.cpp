...
void megabox::switchTab(int index)//, ITEMMODE mode)
{
    ...
    case TAB_MGX:
        ...
        } else
        {
            ...
            } else if( m_eMgxListOrderType == TYPE_START_ORDER) {
...
void megabox::slotEfProcKill()
{
    ...
    if( cfg->cfg_cKeyToKill ) {
        if( showQ( lng->tr("POP_KILL"), lng->tr("FT_CONFIRM_KILL"), 1) ) {
            proc_kill();
        }
    }
}
...
void megabox::slotEfFmDel()
{
    ...
    if( cfg->cfg_cKeyToKill ) {
        if( showQ( lng->tr("FT_DELETE"), lng->tr("FT_CONFIRM_DELETE"), 1) ) {
            fm_del();
        }
    }
}
...
bool megabox::eventFilter(QObject *obj, QEvent *pEvent)
{
    ...
    if (QEvent::KeyRelease == pEvent->type())
    {
        if( pKey->key() == KEYCODE_CLEAR ) {
            ...
            if(obj == fmListbox || obj == fmListbox2) {
                slotEfFmDel();
                return true;
            }
        }
        ...
        if( pKey->key() == KEYCODE_RED ) {
            slotEfHide(); /*return true;*/
        }
